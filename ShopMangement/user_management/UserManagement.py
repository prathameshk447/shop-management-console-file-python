...
class UserManagement:
    al = []  # Observe List syntax
    file_path = r"C:\Users\user\PycharmProjects\ShopManagement\user_management\Users.csv"
    ...
    @staticmethod
    def add_user():
        # Observer the object creation syntax. We dont use new keyword in python. Why ? read ReadMe file.
        user_object = User()
    ...
    def delete_user(user_name):
        for user in UserManagement.al[:]:
            if user.userName.lower() == user_name.lower():
                UserManagement.al.remove(user)  # Observer how to remove item from List
    ...
    def load_data_from_file():
        if os.path.exists(UserManagement.file_path):
            with open(UserManagement.file_path, "r") as file:
                for line in file:
                    user_data = line.strip().split(",")  # Observer String split function
                    if len(user_data) > 3:  # Observer String length function
                        # Parameterised constructor call
                        user = User(user_data[2], user_data[0], user_data[1], "", user_data[3])
                        UserManagement.al.append(user)  # Observer how to add data in list
